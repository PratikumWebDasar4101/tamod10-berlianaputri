	...
	class controller{
		...
		function dashboard(){
			...
			if (mysqli_num_rows($result) > 0) {
				while ($row = mysqli_fetch_assoc($result)) {
					...
					echo "<td>" . $row['namablkng']  . "</td>";
					echo "<td>" . $row['email']  . "</td>";
					echo "<td>" . $row['kelas']  . "</td>";
					echo "<td>" . $row['hobi']  . "</td>";
					echo "<td>" . $row['genrefilm']  . "</td>";
		...
		function newData(){
			...
			require_once 'koneksi.php';
				...
				$hobi = implode(", ", $hobby);
				$film = implode(", ", $genrefilm);
				$tempatwisata = implode(", ", $wisata);	

				$sql = "INSERT INTO `mahasiswa`(`nim`, `namadepan`, `namablkng`, `email`, `kelas`, `hobi`, `genrefilm`, `wisata`, `ttl`)
						VALUES ('$nim', '$namadepan', '$namablkng', '$email', '$kelas', '$hobi', '$film', '$tempatwisata', '$ttl')";
		...
		function edit(){
			if (isset($_POST['editdata'])) {
				...
				$sql = "UPDATE `mahasiswa` SET `namadepan`='$namadepan',`namablkng`='$namablkng',`email`='$email',
						`kelas`='$kelas',`hobi`='$hobi',`genrefilm`='$film',`wisata`='$tempatwisata',`ttl`='$ttl' WHERE 
						`nim`='$nim'";
